...
export default function EditListing() {
    ...
    const [localizacionGuardada, setlocalizacionGuardada] = useState({ lat: 10.36402678444531, lng: -85.3908877959475 });
    ...
    useEffect(() => {
        async function fetchListing() {
          ...
          if (docSnap.exists()) {
            ...
            setListing(data);
            setFormData({ ...data });
            setlocalizacionGuardada({ lat: data.geolocation.lat, lng: data.geolocation.lng });
            setGeolocation({ lat: data.geolocation.lat, lng: data.geolocation.lng });
    ...
    return (
        ...
        <main className='max-w-md px-2 mx-auto sm:max-w-lg md:max-w-xl lg:max-w-2xl xl:max-w-4xl'>
            <h1 className='text-3xl text-center mt-6 font-bold'>Formulario editar propiedades</h1>
            <form onSubmit={onSubmit}>
                ...
                <textarea type="text" id='description' value={description} onChange={onChange}
                    placeholder='Maximo 150 caracteres' required maxLength='150'
                    className='w-full px-4 py-2 text-xl text-gray-700
                    bg-white border border-gray-300 rounded transition
                    duration-150 ease-in-out focus:text-gray-700
                    focus:bg-white focus:border-slate-600 mb-6'/>
                ...
                <div className='flex items-center mb-6'>
                    <div className=''>
                        ...
                        <div className='flex w-full justify-center items-center space-x-6'>
                            ...
                            {type === 'rent' && (
                                <div className=''>
                                    <p className='text-md w-full whitespace-nowrap'>$ / Noche</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className='mb-6'>
                    <p className='text-lg font-semibold'>Ingrese las nuevas imagenes de la Propiedad</p>
                    ...
                    <div className="form-group multi-preview md:w-[100%] lg:w-[94%] mt-3  mb-12 md:mb-6" style={{ display: 'flex', flexWrap: 'nowrap' }}>
                        <ul className="has-scrollbar">
                            {selectedImage &&
                                ...
                                <img className='rounded-2xl transition-transform duration-300 transform hover:scale-105 hover:shadow-lg'
                                 key={index} src={URL.createObjectURL(image)} alt={`Image ${index}`}
                                 style={{ maxWidth: '100%', height: '220px', marginRight: '10px' }}
                                 />
                                )) 
                            }
            ...
            />
            {
                loading && (
                    <div className="mt-1 mb-1">
                        <span className="font-bold text-cyan-700 text-lg">Actualizando Datos... {progress}%</span>
                        <progress
            ...
            duration-150 ease-in-out text-center'>Editar</button>
